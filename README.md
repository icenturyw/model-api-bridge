...
### 2026-06-26 — 健康状态增强 & Bug 修复

- 新增健康状态中文提示：Provider Key / 路由的健康状态现在显示具体原因（密钥无效、无权限、请求频率限制、请求超时、服务器故障、无法连接等），后端从 `last_error` 自动派生 `health_reason` 字段
- 修复模态框关闭/取消按钮不起作用的问题：去除 `type="button"`，改用默认 `type="submit"` 配合 `method="dialog"` 表单自动关闭行为
- 修复登录/首次设置后侧边栏点击无响应的问题：将 `router.onChange` 赋值提取为 `setupRouterOnChange()` 共享函数，确保所有入口路径都正确绑定路由事件
- 精简 AGENTS.md，补充环境变量表和安全说明

### 2026-06-25 — 管理面板 Bug 修复

- 修复模态框中取消/关闭按钮未设置 `type="button"` 导致意外触发 API 创建/编辑请求的问题（注：此修复在 6-26 中改进为更彻底的方案）
- 修复重复登录后侧边栏点击无响应的问题（注：6-26 中重构为共享函数方案）
