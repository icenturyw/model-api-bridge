...
export function createModal() {
    ...
    return `
      <form method="dialog" class="modal-shell" data-form="${isEdit ? 'edit-provider' : 'create-provider'}">
        <div class="card-header">
          ...
          <button type="button" class="ghost-button" value="cancel">关闭</button>
        </div>
        ...
        <menu class="row-actions">
          <button type="button" value="cancel" class="ghost-button">取消</button>
  ...
  function createRouteForm(route = {}, groups = [], providerKeyOptions = []) {
    ...
    const keyOptions = providerKeyOptions.map(k => 
      `<option value="${k.id}" ${String(route.provider_key_id) === String(k.id) ? 'selected' : ''}>${escapeHtml(k.provider_name)} / ${escapeHtml(k.label)} / ${escapeHtml(k.health_status)}</option>`
    ).join('');
